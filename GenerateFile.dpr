...
uses
  ...
  uHelpers in 'uHelpers.pas';  // Add this line
...
var
  DriverID, Server, Database, UserName, Password, ColumnConfigFile, SQLQueryFile, OutputFilePrefix, MappingFile, OutputPath, GroupColumn, GroupQueryFile: string;
  SkipJournal: Boolean;
...
begin
  try
    ...
    if DriverID.IsEmpty or Server.IsEmpty or Database.IsEmpty or UserName.IsEmpty or Password.IsEmpty or ColumnConfigFile.IsEmpty or SQLQueryFile.IsEmpty or OutputFilePrefix.IsEmpty or MappingFile.IsEmpty or OutputPath.IsEmpty then
    begin
      ShowUsage;
    end;
    ...
    try
      DataModuleMain.ExtractAndSaveData(DriverID, Server, Database, UserName, Password, ColumnConfigFile, SQLQueryFile, OutputFilePrefix, MappingFile, JourType, GroupColumn, GroupQueryFile, OutputPath, DateStr, SkipJournal);
    finally
      DataModuleMain.Free;
    end;
  except
